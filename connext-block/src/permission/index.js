 ...
 */
function routeMatch(data, base, options = { url: 'path', name: 'name', id: "id", permissions: 'permissions' }) {
    ...
    menu.forEach(item => {
      ...
      try {
        let routerItem = {
          ...
          meta: item.meta, // 路由元信息 定义路由时即可携带的参数，可用来管理每个路由的按钮操作权限, 详细可以看minix下suth.js但是现在更普遍的是做v-auth指令
          component: () => import(`@/pages${_url}/index.vue`) // 路由映射真实视图路径
        };
        routerBox.push(routerItem);
      } catch (err) {
        item[options.url] = '/404'
      }
    });

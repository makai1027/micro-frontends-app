...
<template lang='pug'>
    div.wrapper
        template(v-for='el in routes', v-if='!el.hidden&&el.children && el.children.length')
            el-menu-item(v-if='hasOneShowingChildren(el.children) && !el.children[0].children', :index="el.children[0].path", :key='el.children[0].name', @click="goToUrl(el.module, el.path, el.children[0].path)")
